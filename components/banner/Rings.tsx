...
const ringsPositions = [
    ...
    [0, ringRadius, 0],
    [
        -(ringSize / 2 + ringOffset),
        -(ringSize / 2 + ringOffset) + ringRadius,
        0,
    ],
...
const Rings: React.FC = () => {
    ...
    const recalculateCameraPosition = useCallback(
        (
            ...
            let newX =
                currentPosition.x +
                (-mouseX + currentPosition.x) * parallaxSpeed;
            let newY =
                currentPosition.y +
                (mouseY - currentPosition.y) * parallaxSpeed;

            newX = Math.min(
                Math.max(newX, -cameraOffsetLimit),
                cameraOffsetLimit
            );
            newY = Math.min(
                Math.max(newY, -cameraOffsetLimit),
                cameraOffsetLimit
            );
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            return () => {
                ...
                if (
                    ref.current &&
                    renderer.domElement.parentNode === ref.current
                ) {
